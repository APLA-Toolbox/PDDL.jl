...
substitute(term::Term, subst::Dict{Any, Any}) = error("Not implemented.")
substitute(term::Const, subst::Dict{Any, Any}) = term
substitute(term::Var, subst::Dict{Any, Any}) = get(subst, term, term)
function substitute(term::Compound, subst::Dict{Any, Any})
    args, ident = nothing, true
    for (i, a) in enumerate(term.args)
